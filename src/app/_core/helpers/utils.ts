...
export default class Utils {
    ...
    public static  isBaseResponse<T>(obj: BaseResponse<T>) {
        return "apiError" in obj;
    }

    public static parseDuration(duration: string): number {
        const weekDayHourMinuteRegex = /(\d+)[wW]\s*|(\d+)[dD]\s*|(\d+)[hH]\s*|(\d+)[mM]\s*/;
    
        const match = duration.match(weekDayHourMinuteRegex);
        ...
        const totalMinutes = weeks * 5 * 8 * 60 + days * 8 * 60 + hours * 60 + minutes;
    
        return totalMinutes;
    }
    
    public static computeWeeksDaysHours(totalMinutes: number) {
        const totalWeeks = Math.floor(totalMinutes / (5 * 8 * 60));
        const totalDays = Math.floor(totalMinutes / (8 * 60)) % 5;
        const totalHours = Math.floor(totalMinutes / 60) % 8;
        const remainingMinutes = totalMinutes % 60;
    
        let returnString = '';
        if (totalWeeks) {
            returnString += `${totalWeeks}W`;
        }
        if (totalDays) {
            returnString += ` ${totalDays}d`;
        }
        if (totalHours) {
            returnString += ` ${totalHours}h`;
        }
        if (remainingMinutes) {
            returnString += ` ${remainingMinutes}m`;
        }
        return returnString.trim();
    }

    public static getWorkingDays(startDate, endDate) {
    ...
    public static isTeamLead(token){
        var roles = AuthUtils.getRoles(token);
        return roles.includes('Team Lead') || roles.includes('Manager') || roles.includes('HR');
    }
